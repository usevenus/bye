package main;

use 5.018;

use strict;
use warnings;

use Venus;
use Venus::Test;

my $test = test(__FILE__);

=name

Venus

=cut

$test->for('name');

=tagline

Attributes

=cut

$test->for('tagline');

=abstract

Object Attributes in Venus

=cut

$test->for('abstract');

=synopsis

  package User;

  use Venus::Class;

  attr 'name';
  attr 'email';

  package main;

  my $user = User->new(name => 'Alice');

  $user->email('alice@example.com');

  $user->email;

  # "alice@example.com"

=cut

$test->for('synopsis', sub {
  my ($tryable) = @_;
  my $result = $tryable->result;
  $test->is($result, 'alice@example.com');

  $result
});

=description

Venus provides the C<attr> function for declaring attributes. Each attribute
gets an accessor method that works as both a getter and setter.

=cut

$test->for('description');

=examples

The following are examples of attributes in Venus.

=cut

=example attr.0

Use C<attr> to declare attributes with auto-generated accessors.

=example-1 attr.0

  package Config;

  use Venus::Class;

  attr 'host';
  attr 'port';

  package main;

  my $config = Config->new(host => 'localhost', port => 8080);

  my $result = $config->host . ':' . $config->port;

  # "localhost:8080"

=cut

$test->for('example', 1, 'attr.0', sub {
  my ($tryable) = @_;
  my $result = $tryable->result;
  $test->is($result, 'localhost:8080');

  $result
});

=example attr.1

Accessors act as both getters and setters.

=example-1 attr.1

  package Toggle;

  use Venus::Class;

  attr 'state';

  package main;

  my $toggle = Toggle->new;

  $toggle->state('on');

  my $result = $toggle->state;

  # "on"

=cut

$test->for('example', 1, 'attr.1', sub {
  my ($tryable) = @_;
  my $result = $tryable->result;
  $test->is($result, 'on');

  $result
});

=example attr.2

Setters return the assigned value.

=example-1 attr.2

  package Box;

  use Venus::Class;

  attr 'width';
  attr 'height';

  package main;

  my $box = Box->new(width => 10, height => 20);

  my $result = $box->width * $box->height;

  # 200

=cut

$test->for('example', 1, 'attr.2', sub {
  my ($tryable) = @_;
  my $result = $tryable->result;
  $test->is($result, 200);

  $result
});

=example mask.0

Use C<mask> to declare private attributes accessible only within the class.

=example-1 mask.0

  package Secret;

  use Venus::Class;

  attr 'name';
  mask 'password';

  sub BUILD {
    my ($self, $args) = @_;
    $self->password($args->{password}) if exists $args->{password};
  }

  sub authenticate {
    my ($self, $input) = @_;
    return $self->password eq $input ? 'granted' : 'denied';
  }

  package main;

  my $secret = Secret->new(name => 'admin', password => 's3cr3t');

  my $result = $secret->authenticate('s3cr3t');

  # "granted"

=cut

$test->for('example', 1, 'mask.0', sub {
  my ($tryable) = @_;
  my $result = $tryable->result;
  $test->is($result, 'granted');

  $result
});

=example mask.1

Masked attributes are not visible via introspection.

=example-1 mask.1

  package Credentials;

  use Venus::Class;

  attr 'username';
  mask 'token';

  package main;

  use Venus::Meta;

  my $meta = Venus::Meta->new(name => 'Credentials');

  my $result = join ', ', sort @{$meta->attrs};

  # "username" (token is not listed)

=cut

$test->for('example', 1, 'mask.1', sub {
  my ($tryable) = @_;
  my $result = $tryable->result;
  $test->is($result, 'username');

  $result
});

=example-2 mask.1

  package Credentials;

  use Venus::Class;

  attr 'username';
  mask 'token';

  package main;

  use Venus::Meta;

  my $meta = Venus::Meta->new(name => 'Credentials');

  my $result = join ', ', sort @{$meta->masks};

  # "token" (username is not listed)

=cut

$test->for('example', 2, 'mask.1', sub {
  my ($tryable) = @_;
  my $result = $tryable->result;
  $test->is($result, 'token');

  $result
});

=layout

name
abstract
synopsis
description
examples: example
partials
authors
license
project

=cut

$test->for('layout');

# END

$test->do('render', './topics/attributes.pod')->done;
